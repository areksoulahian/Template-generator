export const generateDotenv = () => {
  ...
  if (answers.database.toLowerCase() === 'mongodb') {
    return (mongoConfig = `dbURL=mymongo
    MONGODB_URI=mongodb://localhost:27017/
MONGODB_DB=mymongo`);
  }
  // PG
  if (answers.database.toLowerCase() === 'pg') {
    return (pgConfig = `PG_HOST=localhost
    PG_PORT=5432
    PG_USER=pg-user
  ...
  if (answers.database.toLowerCase() === 'sqlite3') {
    return (sqlite3Config = `SQLITE_DB_PATH=database.db`);
  }
  // Mysql
  if (answers.database.toLowerCase() === 'mysql') {
    return (mysqlConfig = `MYSQL_HOST=<your_mysql_host>
  ...
  return `PORT=3000
  ${mongoConfig}${sqlite3Config}${pg}${mysqlConfig}`;
};
